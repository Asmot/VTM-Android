	...
	public boolean update(MapPosition pos) {
		...
		if (changed) {
			synchronized (mTilelock) {
				...
				MapTile[] mCurTiles = mCurrentTiles.tiles;
				android.util.Log.e("zxy", "mCurrentTiles size: " + mCurTiles.length);
				for(int i = 0; i < mCurTiles.length; i++) {
					MapTile tile = mCurTiles[i];
					if(tile != null) {
						android.util.Log.e("zxy", tile.toString());
					}
				}
				...
				MapTile[] mTiles = mNewTiles.tiles;
				android.util.Log.e("zxy", "mNewTiles size: " + mTiles.length);
				for(int i = 0; i < mTiles.length; i++) {
					MapTile tile = mTiles[i];
					if(tile != null) {
						android.util.Log.e("zxy", tile.toString());
					}
				}
